    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i, layer in enumerate(self.model.layers):
            if 'conv' not in layer.name:
                continue
            filters_num = len(layer.get_weights()[0][0,0,0])
            for j in range(filters_num):
                filter = layer.get_weights()[0][:, :, 0, j]
                plt.imshow(filter, interpolation="nearest", cmap="gray")
                plt.savefig(str(i) + "_" + str(j) + "_" + str(epoch) + ".png")
...
class pr8:
    ...
    def build_model(self):
        ...
        H = model.fit(self.x_train, self.y_train, batch_size=self.batch_size, epochs=self.epochs, verbose=1, shuffle=True,
            validation_data=(self.x_test, self.y_test), callbacks=[PlotFilters()]
        )
